...
async function toTempArray(blob) {
  ...
  return [...new Int16Array(arrayBuffer)].map(byteToTemp);
}
...
(async () => {
  ...
  const tempInside = await fetch('/api/48h.temp.inside.bin').then(r => r.blob()).then(blob => toTempArray(blob));
  const tempOutside = await fetch('/api/48h.temp.outside.bin').then(r => r.blob()).then(blob => toTempArray(blob));
  const onoff = await fetch('/api/48h.onoff.bin').then(r => r.blob()).then(blob => toBitArray(blob));
  const MINUTE = 60*1000;
  const HOUR = 60*MINUTE;
  const duration = 48*HOUR;
  ...
  new Chart(document.getElementById('chart48h'), config);
